...
function PeopleList({ selected, setSelected }) {
  ...
  const people = roomData?.users
    ?.filter((id) => id != userId)
    ?.map((id) => findUser(id));

  function Person({ firstName, lastName, imageUrl, id }) {
    return (
      <div
        onClick={() => {
      ...
      >
        <div className=" flex align-middle justify-center items-center">
          ...
          <div className=" pl-4 flex-grow gap-1 flex flex-col  justify-center">
            <p className=" font-semibold flex-grow text-start">{firstName + ' ' + lastName} </p>
          </div>
        </div>
      </div>
    );
...
export default function ChatsRemoveFromRoom() {
  const { userId } = useAuth();
  ...
  return (
    <div className=" flex  justify-end mb-4 group">
      <button
        onClick={toggleOpen}
        className=" flex gap-2 text-xs uppercase font-bold  bg-red-700 hover:bg-red-900 text-white   p-2 rounded justify-center items-center"
      >
        <IoAddOutline fontSize={20} /> Delete Member
      </button>
      <div
      ...
      >
        <div className=" bg-white p-4 flex gap-4 flex-col rounded  max-lg:w-1/2 w-1/4">
          ...
          <div className=" flex gap-2 justify-end">
            ...
            <button
              onClick={() =>
                removeMember(selected, room, setError, setSuccess, () => {
                  loadRoom();
                  reloadRooms();
                })
              }
              className="flex h-max gap-2 uppercase text-xs font-semibold bg-red-700 hover:bg-red-900 text-white px-4 py-2 rounded w-max"
            >
              <TbMessageCircle2 className=" " fontSize={16} /> Remove
            </button>
          </div>
        </div>
      </div>
